<?php
	...
	if ($_SESSION["login"] == "admin")
	{
		...
		if ($_GET["page"] == "add")
		{
			echo '<h3>Создание операции по приему наличных</h3>';
			echo '<p>Заполните данные:</p>';
			echo '<div class="login-block">';
			echo '<form name="addclients" action="cashreceiving" method="post">';
			...
			if (isset($_GET["line"]) && empty($_GET["line"]))
			{
				?>
					<script type="text/javascript">
						location.href = "cashreceiving";
					</script>
				<?php
			}
			echo '<h3>Список операций по приему наличных</h3>';
			echo '<table style="width: 100%" id="input_table">';
			echo '	<tr>';
			echo '		<td style="width: 50%; text-align: left;">';
			echo '			<p><a href="cashreceiving?page=add" class="knopka">Выполнить операцию по приему наличных</a></p>';
			echo '		</td>';
			echo '		<td style="width: 50%; text-align: right;">';
			echo '			<form class="form-wrapper cf" name="searchform" action="cashreceiving" method="get">';
